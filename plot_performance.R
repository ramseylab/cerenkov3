...
gwava_result_path <- './experiment_result/gwava_performance_wo_chr5_30_CERENKOV2_1337.tsv'
...
c2_result_path <- './experiment_result/c2_performance_wo_chr5_30_CERENKOV2_1337.tsv'  # C1 + LS
...
c2_result <- c2_result %>% add_column(feat_set = "CERENKOV2") 
...
c1_result$test_AVGRANK <- -1 * c1_result$test_AVGRANK
c1_result$train_AVGRANK <- -1 * c1_result$train_AVGRANK

c3_result$test_AVGRANK <- -1 * c3_result$test_AVGRANK
c3_result$train_AVGRANK <- -1 * c3_result$train_AVGRANK

# GWAVA and C2 results are from R and the AVGRANKs are positive already
# No need to flip the sign 
cv_result <- bind_rows(gwava_result, c1_result, c2_result, c3_result)

# Ensure the order on x-axis
cv_result$feat_set <- factor(cv_result$feat_set, 
							 levels=c("GWAVA", "CERENKOV", "CERENKOV2", "CERENKOV3"))
...
save_plot(out_dir, output_file_name, p_auprc, width=2, height=3)
...
save_plot(out_dir, output_file_name, p_auroc, width=2, height=3)
...
save_plot(out_dir, output_file_name, p_avgrank, width=2, height=3)
