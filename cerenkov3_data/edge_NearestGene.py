...
def make_TSS_start_BED(tss_bed_fn):
    tss_bed = pd.read_csv(tss_bed_fn, sep="\t", header=None, usecols=[0,1,2,3,4,5],
                          dtype={"strand": int}, names=["chrom", "chromStart", "chromEnd", "name", "id", "strand"])
...
if __name__ == "__main__":
    output_dir = get_path("edge/snp-gene/NearestGene")

    snp_dir = get_path("vertex/SNP/OSU18")
    snp_start_bed = make_snp_start_BED(os.path.join(snp_dir, "osu18_SNP.bed"))

    ensembl_dir = get_path("vertex/gene/Ensembl")
    ensembl_TSS_start_bed = make_TSS_start_BED(os.path.join(ensembl_dir, "p2_Ensembl_TSS.bed"))

    entrez_dir = get_path("vertex/gene/Entrez")
    entrez_TSS_start_bed = make_TSS_start_BED(os.path.join(entrez_dir, "p3_Entrez_TSS.bed"))
    ...
    snp_ensembl_map.to_csv(os.path.join(output_dir, "p1_SNP_x_NearestEnsembl.tsv"), sep="\t", header=True, index=False)
    snp_entrez_map.to_csv(os.path.join(output_dir, "p1_SNP_x_NearestEntrez.tsv"), sep="\t", header=True, index=False)
    ...
    e2e_df = pd.read_csv(os.path.join(entrez_dir, "p2_Ensembl_x_Entrez.tsv"), sep="\t")
    ...
    snp_gene_el.to_csv(os.path.join(output_dir, "p2_SNP_x_NearestGene.edgelist"), sep="\t", header=False, index=False)
