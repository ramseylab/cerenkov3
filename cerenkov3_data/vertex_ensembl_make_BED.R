...
## save "chromosome", "transcript_start", "transcript_end", "gene_name", "ensembl_gene_id" and "strand" to a BED file, `ensembl_gene_transcript.bed`
# ensembl_df %>% 
# 	select(chromosome, transcript_start, transcript_end, gene_name, ensembl_gene_id, strand) %>%
# 	distinct(.keep_all = TRUE) %>% 
# 	write.table("p2_Ensembl_transcript.bed", sep = "\t", 
# 				quote = FALSE, row.names = FALSE, col.names = FALSE)
